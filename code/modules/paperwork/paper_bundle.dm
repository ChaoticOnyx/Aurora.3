...
/obj/item/paper_bundle/attackby(obj/item/W as obj, mob/user as mob)
	...
	// merging bundles
		...
		// if(istype(W, /obj/item/pen))
			// show_browser(usr, "", "window=[name]") // TODO: actually does nothing, either fix it so you can write directly to the bundle screen or actually prevent the window from opening until you're done
...
/obj/item/paper_bundle/proc/show_content(mob/user as mob)
	...
	if(istype(pages[page], /obj/item/paper))
		...
		show_browser(user, dat + "<html><head><title>[P.name]</title></head>" + "<body style='overflow:hidden'>" + "<div> <img src='tmp_photo.png' width = '180'" + "[P.scribble ? "<div> Written on the back:<br><i>[P.scribble]</i>" : null]" + "</body></html>", "window=[name]")
